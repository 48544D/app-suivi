        ...
        <div class="content">
            <div class="user-society">
                <?php
                    ...
                    $user_societies = $db->query('select s.id, s.name from society s JOIN user_society us on us.id_society = s.id JOIN cs on s.id = cs.id_society WHERE id_user = ' . $_SESSION['id'] . ' GROUP BY s.id, s.name ORDER BY s.name');
                    ...
                    if ($user_societies->rowCount() > 0) {
                        ...
                        echo '<h1>Vous n\'avez pas de société IR Contribution Sociale</h1>';
                    }
